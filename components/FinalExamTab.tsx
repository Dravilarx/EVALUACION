...
import { Student, Teacher, FinalExam } from '../types';
import { CheckCircleIcon, UsersIcon, CloseIcon, PlusIcon, TrashIcon, AcademicIcon, EditIcon, DocumentTextIcon } from './icons';
import { FINAL_EXAM_RUBRIC } from '../constants';
import { FinalExamService } from '../services/dataService';
...
interface FinalExamTabProps {
    ...
    onUpdate: (exam: FinalExam) => void;
}

const FinalExamTab: React.FC<FinalExamTabProps> = ({ students, teachers, currentUserId, finalExams, onUpdate }) => {
    ...
    const [currentExam, setCurrentExam] = useState<Partial<FinalExam>>({
        caseTopics: [],
        commissionIds: [],
        scores: {},
        comments: ''
    ...
    const handleOpenEvaluation = (exam?: FinalExam) => {
        if (exam) {
            setCurrentExam(exam);
        } else {
            setCurrentExam({
    ...
    const calculation = useMemo(() => {
        ...
        // Max possible weighted score is 4 (if all scores are 4)
        const maxScore = 4;
        const score = totalWeightedScore;
        
        // 60% requirement calculation (Chilean Standard)
        // Max Score = 4
        // Passing Score (60%) = 2.4
        const passingScore = 2.4;
        let finalGrade = 1.0;

        if (score < passingScore) {
            // Linear scale 1.0 to 3.9
            finalGrade = 1 + (score / passingScore) * 3;
        } else {
            // Linear scale 4.0 to 7.0
    ...
    const handleSave = async () => {
        ...
        const examToSave: FinalExam = {
            ...
            status: 'Completed',
            finalScore: parseFloat(calculation.weightedScore.toFixed(2)),
            finalGrade: calculation.finalGrade
        };

        try {
            if (finalExams.find(e => e.id === examToSave.id)) {
                await FinalExamService.update(examToSave);
            } else {
                await FinalExamService.create(examToSave);
            }
            onUpdate(examToSave);
            handleCloseModal();
        } catch (error) {
            ...
            <div className="bg-surface p-5 rounded-xl shadow-sm border border-secondary/20 flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold text-text-primary flex items-center gap-2">
                        <AcademicIcon className="h-6 w-6 text-indigo-600" /> Examen Final de Especialidad
                    </h2>
                    <p className="text-sm text-text-secondary">Evaluación ante comisión de casos clínicos.</p>
                </div>
                {isTeacher && (
                    <button 
                            ...
                            finalExams.map(exam => (
                                <tr key={exam.id} className="hover:bg-background/50">
                                    ...
                                    <td className="p-4">
                                        <div className="flex -space-x-2 overflow-hidden">
                                            {exam.commissionIds.map(tid => (
                                                <div key={tid} className="inline-block h-8 w-8 rounded-full ring-2 ring-white bg-indigo-100 flex items-center justify-center text-indigo-700 text-xs font-bold" title={getTeacherName(tid)}>
                                    ...
                                    <td className="p-4 text-center">
                                        <span className="bg-secondary/10 text-text-secondary px-2 py-1 rounded text-xs font-bold">{exam.caseTopics.length}</span>
                                    </td>
                                    <td className="p-4 text-center">
                                        <span className={`font-extrabold text-lg ${getGradeColor(exam.finalGrade)}`}>
                                            {exam.finalGrade.toFixed(1)}
                                        </span>
                                    </td>
                                    <td className="p-4 text-center">
                                        {isTeacher && (
                                            <button onClick={() => handleOpenEvaluation(exam)} className="text-indigo-600 hover:bg-indigo-50 p-2 rounded transition-colors">
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-surface w-full max-w-6xl max-h-[95vh] rounded-xl shadow-2xl flex flex-col border border-secondary/20">
                        <header className="p-5 border-b border-secondary/20 bg-secondary/5 rounded-t-xl flex justify-between items-center">
                            <h3 className="text-xl font-bold text-text-primary">Evaluación de Examen Final</h3>
                        ...
                        <div className="flex-grow overflow-y-auto p-6 space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select 
                                        ...
                                        className="w-full bg-background border border-secondary/30 rounded-lg p-3 outline-none focus:ring-2 focus:ring-indigo-500"
                                        disabled={!!currentExam.finalGrade && currentExam.finalGrade > 0}
                                    >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2 mb-2">
                                        <input 
                                            type="text" 
                                            placeholder="Tema del caso (ej: ACV Isquémico)" 
                                            value={newCaseTopic}
                                            onChange={(e) => setNewCaseTopic(e.target.value)}
                                            onKeyDown={(e) => e.key === 'Enter' && handleAddCase()}
                                            className="flex-grow bg-background border border-secondary/30 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                                        />
                                        <button onClick={handleAddCase} className="bg-indigo-600 text-white p-1.5 rounded-lg hover:bg-indigo-700 transition-colors"><PlusIcon className="h-5 w-5"/></button>
                                    </div>
                                    <div className="bg-background border border-secondary/30 rounded-lg p-3 max-h-40 overflow-y-auto space-y-1">
                                        ...
                                        {(currentExam.caseTopics || []).map((topic, idx) => (
                                            <div key={idx} className="flex justify-between items-center bg-surface p-2 rounded border border-secondary/10">
                                                <span className="text-sm font-medium"><span className="text-indigo-600 font-bold mr-2">{idx + 1}.</span> {topic}</span>
                                        ...
                                        <tbody className="divide-y divide-secondary/10">
                                            {FINAL_EXAM_RUBRIC.map(item => (
                                                <tr key={item.id} className="hover:bg-background/50">
                                                    <td className="p-3">
                                                        <p className="font-bold text-indigo-900">{item.dimension}</p>
                                                                ...
                                                                <button
                                                                    ...
                                                                    className={`w-full h-full min-h-[80px] rounded-lg transition-all flex flex-col items-center justify-center p-2 text-xs text-center border-2
                                                                        ${isSelected 
                                                                            ...
                                                                            : 'bg-transparent border-transparent hover:border-indigo-200 hover:bg-indigo-50 text-text-secondary'
                                                                        }
                                                                    `}
                                                                >
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    rows={3}
                                    className="w-full bg-background border border-secondary/30 rounded-lg p-3 outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                        ...
                        <footer className="p-5 border-t border-secondary/20 bg-surface/95 rounded-b-xl flex justify-between items-center sticky bottom-0">
                            <div className="flex items-center gap-6">
                                <div>
                                    <p className="text-xs font-bold text-text-secondary uppercase">Puntaje Ponderado (0-4)</p>
                                    <p className="text-2xl font-bold text-indigo-700">{calculation.weightedScore.toFixed(2)}</p>
                                </div>
                                <div className="h-10 w-px bg-secondary/20"></div>
                                <div>
                                    <p className="text-xs font-bold text-text-secondary uppercase">Nota Final (60% Exigencia)</p>
                                    <p className={`text-4xl font-extrabold ${getGradeColor(calculation.finalGrade)}`}>{calculation.finalGrade.toFixed(1)}</p>
                                </div>
                            </div>
                            <div className="flex gap-3">
                                ...
                                <button onClick={handleSave} className="px-6 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-lg transition-all flex items-center gap-2">
                                    <CheckCircleIcon className="h-5 w-5" /> Guardar Examen
                                </button>
                            </div>
                        </footer>
                    </div>
