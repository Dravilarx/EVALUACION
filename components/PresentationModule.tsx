...
const PresentationModule: React.FC<PresentationModuleProps> = ({ currentUserId, initialStudentId, initialSubjectId, onClearParams }) => {
    ...
    const progress = Math.round((Object.keys(scores).length / PRESENTATION_CRITERIA_LIST.length) * 100);
    ...
    const handleSave = async () => {
        ...
        const missing = PRESENTATION_CRITERIA_LIST.filter(item => !scores[item.id]).map(item => item.id);
        if (missing.length > 0) {
            setMissingItems(missing);
            alert(`Faltan ${missing.length} criterios por evaluar. Están marcados en rojo.`);
            return;
        }
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-3xl font-bold text-text-primary flex items-center gap-2">
                        <ScreenIcon className="h-8 w-8 text-primary" /> Rúbrica de Presentación
                    </h2>
                    <p className="text-text-secondary">Evaluación de seminarios, casos clínicos y exposiciones.</p>
                </div>
                    ...
                    <div className="bg-surface p-6 rounded-xl shadow-sm border border-secondary/20 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select 
                                ...
                                disabled={!isAdmin || !!initialStudentId}
                            >
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead className="bg-secondary/10 border-b border-secondary/20">
                                    <tr>
                                        <th className="p-4 text-left min-w-[300px] text-text-primary font-bold">Criterio</th>
                                        {EVALUATION_SCALE.map(s => (
                                            <th key={s.value} className="p-2 text-center min-w-[80px]">
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    <span className="text-[10px] uppercase text-text-secondary font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-[80px]" title={s.label}>{s.label}</span>
                                                </div>
                                            </th>
                                        ))}
                                    </tr>
                                ...
                                <tbody className="divide-y divide-secondary/10">
                                    {PRESENTATION_CRITERIA_LIST.map((item, index) => {
